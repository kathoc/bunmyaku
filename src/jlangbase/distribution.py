...
def install(home, selected, update=False, dry_run=False):
    ...
    locations = []
    if "codex" in targets:
        locations.append(home / ".agents" / "skills" / "japanese-discovery-writing")
    if "claude" in targets:
        locations.append(home / ".claude" / "skills" / "japanese-discovery-writing")
    usage = (f"# この端末の設定\n\n実行コマンド: `{invocation}`\n\n"
    ...
    for location in locations:
        files[location / "SKILL.md"] = skill.encode("utf-8")
