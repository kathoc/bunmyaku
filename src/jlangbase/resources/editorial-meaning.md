...
前の文で何が分かった状態なのかを起点に次の文を書く。主語・条件・対象を同時に総入れ替えせず、引き継ぐ意味を残す。一文ごとに新しい発見を要求しない。ひとつの意味を伝えたら、必要な理由や説明を添え、理解できるところまで書いてから次へ進む。短い文でも、未説明の意味を列挙すれば負担になる。例を足す前にeditorial-explanation.mdの直接説明との比較を行う。
...
反復は回数だけで削らない。新情報がなくても、同じ意味が実感できる、前の説明が分かる、印象が深まるなら残す。echoの記録はその判断を扱うもので、反復を珍しい例外にする制約ではない。同じ対象の名前を次々に変えて別の概念に見せない。列挙が不要なら代表や始点と終点で伝えるが、必要な根拠や条件は省かない。
