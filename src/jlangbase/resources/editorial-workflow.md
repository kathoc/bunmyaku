...
文末はeditorial-register.mdに従う。自然な敬体なら「す」が続いても残す。語尾の種類を増やすために、方針の報告を常体の独り言や読者への勧誘へ変えない。理解に役立つ反復は保ち、不要な重複と文の切り方を先に考える。

## 語りの具体性

「AI臭い」「生々しさがない」と指摘された場合はeditorial-voice.mdに従う。短文や常体を増やす前に、実際の出来事・指摘・判断の経緯が抽象的なまとめに置き換わっていないか扱う。語り手の体験を捏造せず、必要な説明は残す。まず局所的な比較稿で方向性を確かめる。

## 節と段落を分ける編集v3

editorial-sections.mdに従う。v3以降ではparagraph_boundariesとsection_layoutを使う。小見出しは本文から再考し、句読点なし・13文字以内。段落の役割と境界理由を記録するが、段落文字数やその分散に合否を設けない。

## 説明の採否を扱う編集v4

新規要求はv4。editorial-explanation.mdに従い、explanation_reviewとreader_checksを追加する。直接説明との比較から始め、例を使わない判断も残す。ひとつ理解してから次へ進む説明と、有益な言い換えを許す。読者の実回答はeditor-reader-feedbackで別に保存する。保存済みv1からv3の要求は従来の契約を維持する。
