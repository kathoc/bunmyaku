...
natural-japaneseの執筆指針と検査を同梱しています。別スキルの導入は不要です。通常検査の初回には専用環境へ日本語辞書などを自動取得するため、Pythonのvenv・pip機能と通信が必要です。詳しくは[同梱の日本語執筆と検査](docs/install.md#同梱の日本語執筆と検査)を参照してください。
...
書き上がった文章から、何を残し、何を削るかを考える編集手順も追加しています。単に短くするのではなく、主張を支える根拠や、大事な条件を失わないように扱います。同じ話を別の言葉で伝え直すことと、新しい話を次々に足すことも分けて考えます。

新しく書く文章は、指定がなければ「です・ます」調を標準にします。文体の指定や、原文の調子を残す依頼があれば、そちらを優先します。追加した編集機能の品質評価やテストは、まだ十分にできていません。

## どこで使えるのか

| 使うもの | 使い方 |
| --- | --- |
| Codex | 作文用スキルがCodexのサブエージェントへ執筆と点検を依頼します。 |
| Claude Code | 作文用スキルがClaudeのサブエージェントへ執筆と点検を依頼します。 |
| Ollama | 専用コマンドが同じOllama接続で執筆役・点検役・編集役を呼びます。 |

CodexとClaude Codeでは「○○について書いて」と頼むと、親AIがbunmyakuの手順で指示を作り、利用中の環境のサブエージェントへ渡します。問題があれば理由付きで差し戻し、初稿の合格後に全文編集と再点検を行って原稿を返します。別の生成APIは不要です。スキルの自動選択はAIの判断に依存し、サブエージェント機能が使える環境が必要です。

Ollamaでは、通常の会話すべてに自動適用されるわけではありません。モデルと依頼をまとめたbrief.jsonを用意し、writing-start、writing-runで進めます。最終点検を通過した原稿だけをwriting-handoffで書き出します。[導入と使い方](docs/install.md)に例があります。モデルの点検による合格は、実読者による品質評価とは区別します。
